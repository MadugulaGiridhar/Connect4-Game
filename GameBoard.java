...
public class GameBoard 
{
	 ...
	 * @return a d ual indexed array representing the gameboard
	 */
	...
	public int eval_func(int player) {		
		int playerScore = 0;
		//check horizontally
for( int i = 0; i < 6; i++ ) 
{
	for( int j = 0; j < 4; j++ ) 
	{
		if( ( this.playBoard[ i ][j] == player ) && ( this.playBoard[ i ][ j+1 ] == player ))
		{
			...
			else if( this.playBoard[ i ][ j+2 ] == player ) 
			{
				if( this.playBoard[ i ][ j+3 ] == player)
					playerScore+=8;
					else if( this.playBoard[ i ][ j+3 ] == 0)
		...
		else if( ( this.playBoard[ i ][j+2] == player ) && ( this.playBoard[ i ][ j+3 ] == player ))
		{
			if( this.playBoard[ i ][ j+1] == 0 ) 
			{
				if( this.playBoard[ i ][ j] == player)
					playerScore+=3;
			...
			else if(( this.playBoard[ i ][ j+1 ] == player)&& this.playBoard[ i ][ j ] == 0)	
				playerScore+=3;
		}
		else if(( this.playBoard[ i ][j] == player ) && ( this.playBoard[ i ][ j+1 ] == 0 ))
		{
			if(( this.playBoard[ i ][j+2] == player ) && ( this.playBoard[ i ][ j+3 ] == 0 ))
			{
			   playerScore+=2;
			}
			else if(( this.playBoard[ i ][j+2] == 0 ) && ( this.playBoard[ i ][ j+3 ] == player ))
			{
				   playerScore+=2;
				}
		}
		else if(( this.playBoard[ i ][j] == 0 ) && ( this.playBoard[ i ][ j+1 ] == player ))
		{
			if(( this.playBoard[ i ][j+2] == player ) && ( this.playBoard[ i ][ j+3 ] == 0 ))
			{
			   playerScore+=2;
...
// end check horizontally
		//check vertically
		for( int i = 0; i < 3; i++ ) {
			for( int j = 0; j < 7; j++ ) {
				if(( this.playBoard[ i+2 ][ j ] == player ) &&( this.playBoard[ i+3 ][ j ] == player ) ) {
					if(( this.playBoard[ i+1 ][ j ] == 0 ))
					    playerScore+=2;
					else if(( this.playBoard[ i +1][ j] == player ))
					{   
						if(( this.playBoard[ i ][ j ] == 0 ))
							playerScore+=4;
						else if(( this.playBoard[ i ][ j ] == player))
							playerScore+=8;
					}
				}
			}
		} // end check vertically
...
for( int i = 0; i < 3; i++ ) 
{
	for( int j = 0; j < 4; j++ ) 
	{
		if( ( this.playBoard[ i ][j] == player ) && ( this.playBoard[ i+1 ][ j+1 ] == player ))
		{
			...
			else if( this.playBoard[ i+2 ][ j+2 ] == player ) 
			{
				if( this.playBoard[ i+3 ][ j+3 ] == player)
					playerScore+=8;
					else if( this.playBoard[ i+3 ][ j+3 ] == 0)
		...
		else if( ( this.playBoard[ i+2 ][j+2] == player ) && ( this.playBoard[ i+3 ][ j+3 ] == player ))
		{
			if( this.playBoard[ i+1][ j+1] == 0 ) 
			{
				if( this.playBoard[ i ][ j] == player)
					playerScore+=3;
				else if( this.playBoard[ i ][ j ] == 0)
			...
			else if(( this.playBoard[ i+1 ][ j+1 ] == player)&& this.playBoard[ i ][ j ] == 0)	
				playerScore+=3;
		}
		else if(( this.playBoard[ i ][j] == player ) && ( this.playBoard[ i+1 ][ j+1 ] == 0 ))
		{
			if(( this.playBoard[ i+2 ][j+2] == player ) && ( this.playBoard[ i+3 ][ j+3 ] == 0 ))
			{
			   playerScore+=2;
			}
			else if(( this.playBoard[ i +2][j+2] == 0 ) && ( this.playBoard[ i+3 ][ j+3 ] == player ))
			{
				   playerScore+=2;
				}
		}
		else if(( this.playBoard[ i ][j] == 0 ) && ( this.playBoard[ i+1 ][ j+1 ] == player ))
		{
			if(( this.playBoard[ i+2 ][j+2] == player ) && ( this.playBoard[ i+3 ][ j+3 ] == 0 ))
			{
			   playerScore+=2;
...
// end check diagonally - backs lash ->	\
...
for( int i = 0; i < 3; i++ ) 
{
	for( int j = 0; j < 4; j++ ) 
	{
		if( ( this.playBoard[ i+3 ][j] == player ) && ( this.playBoard[ i+2 ][ j+1 ] == player ))
		{
			...
			else if( this.playBoard[ i+1 ][ j+2 ] == player ) 
			{
				if( this.playBoard[ i ][ j+3 ] == player)
					playerScore+=8;
					else if( this.playBoard[ i ][ j+3 ] == 0)
		...
		else if( ( this.playBoard[ i+1 ][j+2] == player ) && ( this.playBoard[ i ][ j+3 ] == player ))
		{
			if( this.playBoard[ i+2][ j+1] == 0 ) 
			{
				if( this.playBoard[ i+3 ][ j] == player)
					playerScore+=3;
			...
			else if(( this.playBoard[ i+2 ][ j+1 ] == player)&& this.playBoard[ i+3 ][ j ] == 0)	
				playerScore+=3;
		}
		else if(( this.playBoard[ i+3 ][j] == player ) && ( this.playBoard[ i+2 ][ j+1 ] == 0 ))
		{
			if(( this.playBoard[ i+1 ][j+2] == player ) && ( this.playBoard[ i ][ j+3 ] == 0 ))
			{
			   playerScore+=2;
			}
			else if(( this.playBoard[ i+1 ][j+2] == 0 ) && ( this.playBoard[ i ][ j+3 ] == player ))
			{
				   playerScore+=2;
				}
		}
		else if(( this.playBoard[ i+3 ][j] == 0 ) && ( this.playBoard[ i+2 ][ j+1 ] == player ))
		{
			if(( this.playBoard[ i+1 ][j+2] == player ) && ( this.playBoard[ i ][ j+3 ] == 0 ))
			{
			   playerScore+=2;
 ...
 }				//end check diagonally - forward slash -> /
		return playerScore;
	}    
} 
